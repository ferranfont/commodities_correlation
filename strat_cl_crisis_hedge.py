"""
Crude Oil Crisis Hedging Strategy
Comprar petróleo crudo (CL) durante períodos de crisis como hedging/refugio seguro
Basado en comportamiento histórico del petróleo durante turbulencias de mercado
"""
...
class CrudeCrisisHedgeStrategy:
    ...
    def _define_crisis_periods(self):
        """Definir períodos de crisis donde el petróleo puede actuar como refugio o volatilidad"""
        return [
            {
    ...
    def load_data(self):
        """Cargar datos de petróleo crudo y NASDAQ para comparación"""
        print("Cargando datos...")
        
        try:
            # Cargar petróleo crudo (CL)
            oil_df = pd.read_csv('data/CL.csv')
            oil_df['Date'] = pd.to_datetime(oil_df['Date'], utc=True)
            oil_df.set_index('Date', inplace=True)
            oil_df = oil_df[oil_df.index.dayofweek < 5]  # Solo weekdays
            ...
            # Alinear datos
            aligned_oil, aligned_nasdaq = oil_df['Close'].align(nasdaq_df['Close'], join='inner')
            
            # Crear DataFrame combinado
            self.data = pd.DataFrame({
                'oil_close': aligned_oil,
    ...
    def calculate_position_size(self, crisis_severity, available_capital, current_oil_price):
        """Calcular tamaño de posición basado en severidad de crisis"""
        # Petróleo crudo contracts: $1000 por punto (1000 barriles), mínimo 1 contrato
        
        severity_multipliers = {
            'high': 0.10,    # 10% del capital en crisis severas (reducido por volatilidad)
            'medium': 0.08,  # 8% del capital en crisis medias
            'low': 0.05      # 5% del capital en crisis leves
        }
        
        multiplier = severity_multipliers.get(crisis_severity, 0.08)
        position_value = available_capital * multiplier
        
        # Manejar precios negativos y extrema volatilidad
        if current_oil_price <= 0:
            # Para precios negativos o cero, usar posición fija mínima
            return 1
        
        # Usar contratos más pequeños para reducir riesgo (mini contratos = $10 por punto)
        contract_value = abs(current_oil_price) * 10  # Mini contrato reducido
        
        # Limitar el número de contratos para controlar riesgo
        contracts = max(1, min(5, int(position_value / contract_value)))
        return contracts
    ...
    def run_backtest(self):
        ...
        crisis_oil_return = 0.0      # Yellow: retorno petróleo desde reset
        ...
        for i, (date, row) in enumerate(self.data.iterrows()):
            ...
            if crisis_active and position == 0:
                contracts = self.calculate_position_size(crisis_severity, portfolio_cash, current_oil)
                if contracts > 0:
                    position = contracts
                    entry_price = current_oil
                    entry_date = date
                    
                    # Manejar margen para precios negativos
                    if current_oil <= 0:
                        # Para precios negativos, usar margen fijo
                        used_margin = contracts * 1000  # Margen fijo de $1000 por contrato
                    else:
                        # Margen normal basado en precio
                        used_margin = contracts * abs(current_oil) * 0.1  # Margen 10%
                    
                    # Verificar que tenemos suficiente capital
                    if used_margin <= portfolio_cash:
                        portfolio_cash -= used_margin
                        print(f"[ENTRY] {date.date()}: Comprando {contracts} contratos petróleo a ${current_oil:.2f}")
            ...
            # SALIR DE POSICIÓN: Al final de crisis o stop loss (ajustar condición para precios negativos)
            elif position > 0 and (not crisis_active or (entry_price > 0 and current_oil < entry_price * 0.95) or (entry_price <= 0 and current_oil < entry_price - 5)):
                # Calcular P&L con manejo de precios negativos
                price_diff = current_oil - entry_price
                trade_pnl = position * price_diff * 10  # $10 por punto por contrato (reducido)
                
                # Limitar P&L extremos para evitar distorsiones
                max_pnl = abs(entry_price) * position * 50  # Limitar a 50x el precio de entrada por contrato
                if abs(trade_pnl) > max_pnl:
                    trade_pnl = max_pnl if trade_pnl > 0 else -max_pnl
                
                # Devolver margen
                if entry_price <= 0:
                    used_margin = position * 1000  # Margen fijo para precios negativos
                else:
                    used_margin = position * abs(entry_price) * 0.1
            ...
            if position > 0:
                price_diff = current_oil - entry_price
                unrealized_pnl = position * price_diff * 10  # $10 por punto por contrato (reducido)
                
                # Limitar P&L no realizado extremo
                max_unrealized = abs(entry_price) * position * 50
                if abs(unrealized_pnl) > max_unrealized:
                    unrealized_pnl = max_unrealized if unrealized_pnl > 0 else -max_unrealized
                
                current_portfolio_value += unrealized_pnl
            
            # Calcular retorno del día para petróleo
            oil_daily_return = 0
            if i > 0:
                prev_oil_value = self.equity_curve[i-1]['oil_strategy'] if i > 0 and len(self.equity_curve) > 0 else self.initial_capital
                oil_daily_return = (current_portfolio_value - prev_oil_value) / prev_oil_value if prev_oil_value > 0 else 0
            ...
            if crisis_active and not in_crisis:
                ...
                crisis_oil_return = crisis_reset_point         # Yellow empieza desde blue
            ...
            if in_crisis:
                ...
                crisis_oil_return += oil_daily_return * 100        # Yellow: petróleo desde reset
                crisis_nasdaq_return += nasdaq_daily_return * 100    # Blue: nasdaq desde reset
                crisis_combined_return += (oil_daily_return + nasdaq_daily_return) * 100  # Green: petróleo + nasdaq desde reset
            
            # Calcular returns en porcentaje desde el inicio
            oil_return_pct = (current_portfolio_value / self.initial_capital - 1) * 100
            ...
            if in_crisis:
                ...
                yellow_line_return = crisis_oil_return        # Yellow desde reset
    ...
    def calculate_metrics(self):
        ...
        final_value = equity_df['oil_strategy'].iloc[-1]
        ...
        daily_returns = equity_df['oil_strategy'].pct_change().dropna()
        volatility = daily_returns.std() * np.sqrt(252) * 100
        sharpe = (daily_returns.mean() * 252 / daily_returns.std()) if daily_returns.std() > 0 else 0
        
        # Max Drawdown
        rolling_max = equity_df['oil_strategy'].expanding().max()
        drawdown = (equity_df['oil_strategy'] / rolling_max - 1) * 100
    ...
    def create_chart(self):
        ...
        for i, crisis in enumerate(self.crisis_periods):
            ...
            if len(yellow_segment) > 0:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    mode='lines',
                    name='Crude Oil' if show_legend else None,
                    showlegend=show_legend,
                    line=dict(color='gold', width=1),
                    hovertemplate='<b>Crude Oil Strategy</b><br>' +
                              'Date: %{x}<br>' +
                              'Return: %{y:.1f}%<br>' +
        ...
        # Layout
        fig.update_layout(
            title='Crisis Strategy Analysis: Crude Oil + NASDAQ Combined (% Returns)',
            xaxis_title='Date',
            yaxis_title='Return (%)',
        ...
        # Guardar
        filename = 'charts/crude_oil_crisis_hedge_strategy.html'
    ...
    def print_results(self):
        ...
        print("\n" + "="*80)
        print("CRUDE OIL CRISIS HEDGING STRATEGY - RESULTADOS")
        print("="*80)
        ...
        print(f"  Capital Final (Oil):   ${metrics['final_value']:,.0f}")
        print(f"  Retorno Oil Only:      {metrics['total_return']:+.2f}%")
        print(f"  Retorno Blue Line (NASDAQ Continuous): {metrics['blue_line_return']:+.2f}%")
        print(f"  Retorno NASDAQ:        {metrics['nasdaq_return']:+.2f}%")
        print(f"  Alpha Oil vs NASDAQ:   {metrics['alpha']:+.2f}%")
    ...
    def create_performance_table(self):
        ...
        # NASDAQ + Oil Combined Strategy (Blue Line)
        ...
        combined_daily_returns = equity_df['oil_strategy'].pct_change().dropna() # Usar como proxy
...
<head>
    ...
    <title>📊 Comparativa Rendimiento - NASDAQ vs Estrategia Crisis Petróleo</title>
    <style>
        body {{
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p><strong>NASDAQ Puro vs Estrategia Combinada Crisis Petróleo</strong></p>
...
"""
        ...
        <div class="summary-box">
            ...
            <p><strong>Estrategia Combinada Petróleo</strong> {'superó a' if outperformance > 0 else 'no superó a'} NASDAQ puro por <strong>{abs(outperformance):.1f}%</strong>, 
        ...
        <div class="footer">
            <p>📊 Análisis Estrategia Crisis Petróleo Crudo | 
        ...
        table_filename = 'charts/crude_oil_performance_comparison_table.html'
...
def main():
    """Ejecutar estrategia de hedging con petróleo crudo"""
    print("CRUDE OIL CRISIS HEDGING STRATEGY")
    print("="*40)
    
    strategy = CrudeCrisisHedgeStrategy(initial_capital=10000)
